...
class BaseModule(torch.nn.Module):
    ...
    def __init__(self, config: Union[Dict, BaseConfig]) -> None:
        ...
        self.criterion = torch.nn.MarginRankingLoss(
            margin=self.margin_loss,
            reduction='mean' if self.margin_ranking_loss_size_average else 'sum'
        )

        # Entity dimensions
        #: The number of entities input_data_guo the knowledge graph
        self.num_entities = config.number_entities
        #: The number of unique relation types input_data_guo the knowledge graph
